...
class urp:
    ...
    def get_recentdata(self):
        data = []
        req = self.s.get(self.get_recentdata_url, headers = self.headers, timeout=2)
        text = req.text
        soup = BeautifulSoup(text)
    ...
    def testInfo(self):
        ...
        if testlist == []:
            ...
            for testdata in testlist:
                ...
                if len(str(testDate.string)) > 3:
                    format_testDate = datetime.strptime(str(testDate.string)+" "+str(testTime.string[:5]), '%Y-%m-%d %H:%M')
                    if datetime.today() - timedelta(hours=0.5) < format_testDate:
		        if u'开发区' in testSchool.string:
			    testPlace = testRoom.string
		        else:
			    testPlace = testBulding.string + testRoom.string
                        data = [testName.string.strip(), testPlace, format_testDate, testTime.string, testSeat.string]
        ...
        if len(test_list) > 0:
            ...
            for data in test_list:
                test_str = u'科目: %s\n教室: %s\n日期: %s\n时间: %s\n座位: %s'\
                            %(data[0] ,data[1] ,str(data[2])[0:10], data[3], data[4])
